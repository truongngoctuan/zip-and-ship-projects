...
exports.exec = function progress(inFolder, outFolder) {
    ...
    dirs.forEach(function (dir) {
        ...
        //     && dir.indexOf('Amaris') != -1) {
        //     zipThatFolderForMe(path.join(inFolder, '/packages', dir), outFolder);
        // }

        if (dir.indexOf('Amaris') != -1) {
            CopyFileNupkg(path.join(inFolder, '/packages', dir), outFolder);
        }
...
function getFiles(srcpath, blobPattern = '/**/*.*') {
    var filesWithPath = glob.sync(srcpath + blobPattern, {
        base: srcpath
    }); //TODO: improve
...
function CopyFileNupkg(folderName, outFolder) {
    var files = getFiles(folderName, '/*.*');
    files.forEach(function (file) {
        if (file.indexOf('.nupkg') != -1) {
            fsx.copySync(
                path.join(folderName, file),
                path.join(outFolder, file)
                );
            console.log('nuget package: ' + file);
        }
    });
}
